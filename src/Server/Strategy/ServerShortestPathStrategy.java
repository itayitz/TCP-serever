...
public class ServerShortestPathStrategy implements IServerStrategy {
    ...
    public List<Queue<Index>> findAllPaths(Matrix matrix, Index startIndex, Index targetIndex){
        ...
        Set<NodesData<Index>> nodesDataBFS = bfSvisit.traverse(traversableMatrix);
        if(!nodesDataBFS.contains(bfSvisit.getConcreteDataBFS(targetIndex))){
            return null;
        }
        Queue<NodesData<Index>> checkList = new LinkedList<>();
        ...
        while (!checkList.isEmpty()){
            NodesData<Index> temp = checkList.poll();
    ...
    public Matrix readMatrixFromFile() throws FileNotFoundException {
        ...
        int[][] myMatrix = new int[maxColumn][maxColumn];
