...
//        Matrix matrix = new Matrix(1000,1000);
    ...
    public List<HashSet<Index>>  connectSCC (HashSet<HashSet<HashSet<Index>>> submatricesSccs,Matrix matrix){
        ...
        return result;
        /*
        HashSet<HashSet<Index>> tempRes = new HashSet<>();
        for(HashSet<HashSet<Index>> subMatrixScc1 : scc1) {
            System.out.println(counter++);
            for (HashSet<Index> connectedComponentOfScc1 : subMatrixScc1) {
                HashSet<Index> newConnectedScc = new HashSet<Index>();
                for (Index index : connectedComponentOfScc1) {
                    for (HashSet<HashSet<Index>> subMatrixScc2 : scc2) {
                        for (HashSet<Index> connectedComponentOfScc2 : subMatrixScc2) {
                            if (connectedComponentOfScc2.contains(index)) {
                                newConnectedScc.addAll(connectedComponentOfScc2);
                                newConnectedScc.addAll(connectedComponentOfScc1);
                            }
                        }
                    }
                }
                tempRes.add(newConnectedScc);
            }
        }

        HashSet<HashSet<Index>> copyTempResult = new HashSet<>(tempRes);
        List<HashSet<Index>> finalResult = new ArrayList<>();
        for(HashSet<Index> tempScc : tempRes){
            HashSet<Index> newScc = new HashSet<Index>();
            for(Index index : tempScc){
                for(HashSet<Index> copyTempScc : copyTempResult){
                    if(copyTempScc.contains(index)){
                        newScc.addAll(copyTempScc);
                        newScc.addAll(tempScc);
                    }
                }
            }
            finalResult.add(newScc);
        }
    ...
    public HashSet<HashSet<Index>> initializeSubMatrix (Matrix matrix,int bottomRow,int topRow,int bottomCol,int topCol){
        ...
        for(HashSet<Index> scc : SCCwithTempIndices){
            SCCoriginalIndices.add(converteToOriginalIndices(scc,bottomRow,bottomCol));
        }
    ...
    public HashSet<Index> converteToOriginalIndices (HashSet<Index> scc,int addToRow,int addToCol){
...
//        Comparator<HashSet<Index>> comparator = (h1, h2) -> Integer.compare(h1.size(), h2.size());
//        SCC = (HashSet<HashSet<Index>>) SCC.stream().sorted(comparator).collect(Collectors.toSet());
        return SCC;
    }
